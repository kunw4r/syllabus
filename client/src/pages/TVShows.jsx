          ...
          {hero?.backdrop_path && (
            <div className="relative -mx-6 md:-mx-10 -mt-2 mb-10 h-[60vh] min-h-[400px] overflow-hidden rounded-b-3xl">
              ...
              <div className="absolute bottom-10 left-6 md:left-10 max-w-lg">
                <h1 className="text-3xl md:text-5xl font-black mb-3 leading-tight">{hero.name}</h1>
                ...
                <div className="flex gap-3">
                  <button onClick={() => navigate(`/details/tv/${hero.id}`)} className="bg-white text-black font-semibold px-5 py-2.5 rounded-xl text-sm hover:bg-white/90 transition-colors">More Info</button>
            ...
            <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
              {trending.slice(0, 10).map((item, i) => (
                <div key={item.id} className="flex-shrink-0 flex items-end cursor-pointer group/card" onClick={() => navigate(`/details/tv/${item.id}`)}>
                  ...
                  {item.poster_path && <img src={`${TMDB_IMG}/w300${item.poster_path}`} alt={item.name} className="h-40 rounded-xl relative z-10 shadow-lg group-hover/card:scale-105 transition-transform duration-300" />}
                </div>
              ))}
            </div>
          </div>
