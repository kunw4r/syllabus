...
import { getRatingHex, getRatingGradient, getRatingBg } from '@/lib/utils/rating-colors';
  ...
  return (
    ...
    >
      ...
      {(item.unified_rating || item.vote_average) > 0 && (
        <div className="absolute top-2 right-2 backdrop-blur-md rounded-lg px-1.5 py-0.5 flex items-center gap-1 text-xs font-semibold" style={{ background: getRatingBg(Number(item.unified_rating ?? item.vote_average)) }}>
          <Star size={12} className="fill-current text-white" />
          <span className="text-white">{Number(item.unified_rating ?? item.vote_average).toFixed(1)}</span>
        </div>
      )}
      {item._source && (
                  ...
                  {pinnedFavourites.map((item) => (
                    ...
                    >
                      <div className="aspect-[2/3] rounded-xl overflow-hidden ring-2 ring-gold/30 group-hover:ring-gold/60 transition-all duration-300 group-hover:scale-[1.03] group-hover:shadow-xl group-hover:shadow-gold/10 relative">
                        ...
                        <div className="absolute top-2 right-2 backdrop-blur-sm rounded-lg px-2 py-0.5 flex items-center gap-1" style={{ background: getRatingBg(Number(item.user_rating)) }}>
                          <Star size={12} className="fill-current text-white" />
              ...
              {filteredItems.map((item) => (
                ...
                >
                  ...
                  {item.external_rating > 0 && (
                    <div className="absolute top-2.5 right-2.5 backdrop-blur-md rounded-lg px-1.5 py-0.5 flex items-center gap-1 text-xs font-semibold" style={{ background: getRatingBg(Number(item.external_rating)) }}>
                      <Star size={12} className="fill-current text-white" />
                      <span className="text-white">{Number(item.external_rating).toFixed(1)}</span>
                    </div>
                  )}
                  {item.user_rating > 0 && (
