...
export function getRatingHex(val: number): string {
  ...
  if (val < 10) return '#0ea5e9';   // sky blue — elite
  return '#2563eb';                  // pure blue — perfect 10
}
...
export function getRatingBg(val: number): string {
  ...
  // 9–9.9: dark green fading into sky blue
  if (val < 10) return 'linear-gradient(135deg, #166534, #0ea5e9)';
  // Perfect 10: solid blue
  return '#2563eb';
}
